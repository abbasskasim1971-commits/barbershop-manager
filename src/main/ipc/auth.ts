...
export function registerAuthHandlers(): void {
  ...
  ipcMain.handle('auth:changePassword', async (_event, sessionId: string, oldPassword: string, newPassword: string) => {
    try {
      ...
      beginTransaction();
      try {
        runSql('UPDATE users SET password_hash = ?, updated_at = ? WHERE id = ?', [newPasswordHash, getUtcNow(), userId] as BindParams);
  ...
  ipcMain.handle('auth:setPin', async (_event, sessionId: string, pin: string) => {
    try {
      ...
      runSql('UPDATE users SET pin_hash = ?, updated_at = ? WHERE id = ?', [pinHash, getUtcNow(), userId] as BindParams);
  ...
  ipcMain.handle('auth:deactivateUser', async (_event, sessionId: string, userId: number) => {
    try {
      ...
      beginTransaction();
      try {
        runSql('UPDATE users SET is_active = 0, updated_at = ? WHERE id = ?', [getUtcNow(), userId] as BindParams);
        runSql('DELETE FROM user_sessions WHERE user_id = ?', [userId] as BindParams);
        commitTransaction();
      } catch (txError) {
        rollbackTransaction();
