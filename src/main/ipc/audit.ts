...
export function registerAuditHandlers(): void {
  ...
  ipcMain.handle("get-events", (_event, sessionId: string, limit: number, offset: number) => {
    ...
    if (!session) return [];
    return mapEvents(
      runQuery("SELECT * FROM system_events ORDER BY timestamp DESC LIMIT ? OFFSET ?", [
        limit,
        offset,
      ] as BindParams),
    );
  ...
  ipcMain.handle(
    "auditLog:getByEntity",
    async (_event, sessionId: string, entityType: string, entityId: number) => {
      const session = requireAuth(sessionId, ["owner", "manager"]);
      if (!session) return [];
      return mapAuditEntries(
        runQuery(
