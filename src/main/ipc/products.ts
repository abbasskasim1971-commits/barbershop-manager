...
export function registerProductHandlers(): void {
  ipcMain.handle(
    ...
    async (_event, sessionId: string, limit = 100, offset = 0, includeDeleted = false) => {
      ...
      return mapProducts(
        runQuery(`SELECT * FROM products ${whereClause} ORDER BY name LIMIT ? OFFSET ?`, [
          limit,
          offset,
        ] as BindParams),
      );
  ...
  ipcMain.handle("products:getLowStock", async (_event, sessionId: string) => {
    ...
    if (!session) return [];
    return mapProducts(
      runQuery("SELECT * FROM products WHERE quantity < low_stock_threshold AND is_deleted = 0"),
    );
  });
  ...
  ipcMain.handle("products:getById", async (_event, sessionId: string, id: number) => {
    ...
    const row = runOne("SELECT * FROM products WHERE id = ? AND is_deleted = 0", [
      id,
    ] as BindParams);
  ...
  ipcMain.handle(
    ...
    async (_event, sessionId: string, productId: number, newQuantity: number) => {
      const session = requireAuth(sessionId, ["owner", "manager"]);
      if (!session) return { success: false, error: "Unauthorized" };
      ...
      const oldQty = oldProduct[4] as number;
      const result = runSql("UPDATE products SET quantity = ?, updated_at = ? WHERE id = ?", [
        newQuantity,
        getUtcNow(),
        productId,
      ] as BindParams);
  ...
  ipcMain.handle(
    "products:addStock",
    async (_event, sessionId: string, productId: number, quantity: number) => {
      const session = requireAuth(sessionId, ["owner", "manager"]);
      if (!session) return { success: false, error: "Unauthorized" };
      ...
      logSystemEvent(
        "inventory_added",
        `Added ${quantity} to product ${oldProduct[1] as string} (qty: ${newQty})`,
        1,
      );
  ...
  ipcMain.handle(
    ...
    async (_event, sessionId: string, productId: number, quantity: number) => {
      ...
      logSystemEvent(
        "inventory_removed",
        `Removed ${quantity} from product ${oldProduct[1] as string} (qty: ${newQty})`,
        1,
      );
